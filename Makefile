...
.PHONY: help install checkov-install tflint-install tflint-check tflint-init \
	...
	tofu-init tofu-validate tofu-plan tofu-plan-out \
        ...
        test-terratest test-basic test-snippet test-multiple test-plan test-validate test-content-types \
        terratest-deps terratest-fmt terratest-lint terratest-clean \
...
test-basic: ## Run basic file upload test
	@echo "$(BLUE)Running basic file upload test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxFileBasic
	@echo "$(GREEN)✓ Basic test completed$(NC)"

test-snippet: ## Run snippet upload test
	@echo "$(BLUE)Running snippet upload test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxFileSnippet
	@echo "$(GREEN)✓ Snippet test completed$(NC)"

test-multiple: ## Run multiple files test
	@echo "$(BLUE)Running multiple files test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxFileMultiple
	@echo "$(GREEN)✓ Multiple files test completed$(NC)"

test-plan: ## Run plan-only test
	@echo "$(BLUE)Running plan-only test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxFilePlanOnly
	@echo "$(GREEN)✓ Plan-only test completed$(NC)"

test-validate: ## Run validation test
	@echo "$(BLUE)Running validation test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxFileValidation
	@echo "$(GREEN)✓ Validation test completed$(NC)"

test-content-types: ## Run content types validation test
	@echo "$(BLUE)Running content types validation test...$(NC)"
	cd terratest && go test -v -timeout 30m -run TestProxmoxFileContentTypeValidation
	@echo "$(GREEN)✓ Content types test completed$(NC)"
...
terratest-init: ## Initialize terratest Go module
	...
	cd terratest && go mod init github.com/proxmox/modules/proxmox-terraform-file/terratest || true
...
docs: ## Show module documentation
	@echo "$(BLUE)Proxmox Terraform File Module$(NC)"
	@echo ""
	@echo "This module manages Proxmox files using Terraform or OpenTofu"
	@echo ""
	@echo "Quick Start:"
